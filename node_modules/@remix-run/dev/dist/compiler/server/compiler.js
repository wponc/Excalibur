...
const createEsbuildConfig = (ctx, routes$1, outfile, refs) => {
  let stdin;
  let entryPoints;
  ...
  let plugins = [deprecatedRemixPackage.deprecatedRemixPackagePlugin(ctx), cssBundlePlugin.cssBundlePlugin(refs), cssModuleImports.cssModulesPlugin(ctx, {
    ...
    sideEffects: false
  }), mdx.mdxPlugin(ctx), emptyModules.emptyModulesPlugin(ctx, /\.client(\.[jt]sx?)?$/), routes.serverRouteModulesPlugin(ctx, routes$1), entry.serverEntryModulePlugin(ctx, routes$1), manifest.serverAssetsManifestPlugin(refs, routes$1), bareImports.serverBareModulesPlugin(ctx), external.externalPlugin(/^node:.*/, {
    sideEffects: false
  })];
...
const create = async (ctx, routes, outfile, refs) => {
